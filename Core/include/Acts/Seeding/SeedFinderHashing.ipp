...
void SeedFinderHashing<external_spacepoint_t, platform_t>::createSeedsForGroup(
  ...
  for (auto spM : middleSPs) {
    ...
    for (auto topSP : topSPs) {
      // if (state.checkedBadCompatTopSPSet.count(std::make_pair(*spM, *topSP))){
      //   state.nBadTopSkip += 1;
      //   continue;
      // }
      // if (state.checkedCompatTopSPSet.count(std::make_pair(*spM, *topSP))){
      ...
      if (deltaR < m_config.deltaRMinTopSP) {
        // state.checkedBadCompatTopSPSet.insert(std::make_pair(*spM, *topSP));
        continue;
      }
    ...
    for (auto bottomSP : bottomSPs) {
      // if (state.checkedBadCompatBottomSPSet.count(std::make_pair(*spM, *bottomSP))){
      //   state.nBadBottomSkip += 1;
      //   continue;
      // }
      // if (state.checkedCompatBottomSPSet.count(std::make_pair(*spM, *bottomSP))){
      ...
      if (deltaR > m_config.deltaRMaxBottomSP) {
        // state.checkedBadCompatBottomSPSet.insert(std::make_pair(*spM, *bottomSP));
        continue;
      }
      ...
      // state.checkedCompatBottomSPSet.insert(std::make_pair(*spM, *bottomSP));
      state.compatBottomSP.push_back(bottomSP);
    }
    ...
    for (size_t b = 0; b < numBotSP; b++) {
      ...
      for (size_t t = t0; t < numTopSP; t++) {
        ...
        // if (state.checkedCompatSeedSet.count(std::make_pair(*spM, std::make_pair(*state.compatBottomSP[b], *state.compatTopSP[t])))){
        //   state.nSeedsSkip += 1;
        //   continue;
        // }
        // if (state.checkedCompatSeedSet.count(std::make_pair((static_cast<const IndexSourceLink&>(*((spM.sourceLinks()[0])))).index(), std::make_pair(*state.compatBottomSP[b], *state.compatTopSP[t])))){
        // if (state.checkedCompatSeedSet.count(Seed<external_spacepoint_t>(state.compatBottomSP[b]->sp(), spM->sp(), state.compatTopSP[t]->sp(), 0))){
        //   state.nSeedsSkip += 1;
        //   continue;
        // }
        ...
        if (m_config.useDetailedDoubleMeasurementInfo) {
          ...
          float dU = lt.U - Ub;
          if (dU == 0.) {
            // state.checkedCompatSeedSet.insert(std::make_pair(*spM, std::make_pair(*state.compatBottomSP[b], *state.compatTopSP[t])));
            // state.checkedCompatSeedSet.insert(std::make_pair((static_cast<const IndexSourceLink&>(*((spM.sourceLinks()[0])))).index(), std::make_pair(*state.compatBottomSP[b], *state.compatTopSP[t])));
            // state.checkedCompatSeedSet.insert(Seed<external_spacepoint_t>(state.compatBottomSP[b]->sp(), spM->sp(), state.compatTopSP[t]->sp(), 0));
            continue;
          }
        ...
        if (deltaCotTheta2 > (error2 + scatteringInRegion2)) {
          ...
          if (m_config.skipPreviousTopSP) {
            ...
            if (cotThetaB - cotThetaT < 0) {
              ...
              //   // state.checkedCompatSeedSet.insert(std::make_pair(*spM, std::make_pair(*state.compatBottomSP[b], *state.compatTopSP[tNext])));
              //   state.checkedCompatSeedSet.insert(Seed<external_spacepoint_t>(state.compatBottomSP[b]->sp(), spM->sp(), state.compatTopSP[tNext]->sp(), 0));
              // }
              break;
            }
            t0 = t + 1;
