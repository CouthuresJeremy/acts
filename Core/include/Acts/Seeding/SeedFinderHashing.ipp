...
SeedFinderHashing<external_spacepoint_t, platform_t>::SeedFinderHashing(
    const Acts::SeedFinderConfig<external_spacepoint_t>& config)
    : m_config(config) {
  if (not config.isInInternalUnits) {
    throw std::runtime_error(
        "SeedFinderConfig not in ACTS internal units in SeedFinderHashing");
  }
...
template <template <typename...> typename iterator_t,
          template <typename...> typename container_t, typename sp_range_t>
void SeedFinderHashing<external_spacepoint_t, platform_t>::createSeedsForGroup(
    const Acts::SeedFinderOptions& options, SeedingStateHashing& state,
  ...
  //     // if (state.checkedBadCompatTopSPSet.count(std::make_pair(*spM,
  //     *topSP))){
  //     //   state.nBadTopSkip += 1;
  //     //   continue;
  //     // }
  //     // if (state.checkedCompatTopSPSet.count(std::make_pair(*spM,
  //     *topSP))){
  ...
  //       // state.checkedBadCompatTopSPSet.insert(std::make_pair(*spM,
  //       *topSP)); continue;
  //     }
  ...
  //     // first transform the space point coordinates into a frame such that
  //     the
  ...
  //         // circle into straight lines in the u/v plane the line equation
  //         can
  ...
  //         // in the rotated frame the interaction point is positioned at x =
  //         -rM
  //         // and y ~= impactParam
  //         float uIP = -1. / rM;
  ...
  //         // estimated using du and dv between the two SP bCoef is obtained
  //         by
  ...
  //         // aCoef^2) = 1 / (radius^2) and we can apply the cut on the
  //         curvature if ((bCoef * bCoef) >
  ...
  //       }
  //     }
  //     // state.checkedCompatTopSPSet.insert(std::make_pair(*spM, *topSP));
  //     state.compatTopSP.push_back(topSP);
  //   }
  //   // apply cut on the number of top SP if seedConfirmation is true
  //   SeedFilterState seedFilterState;
  ...
  //     // if (state.checkedBadCompatBottomSPSet.count(std::make_pair(*spM,
  //     *bottomSP))){
  //     //   state.nBadBottomSkip += 1;
  //     //   continue;
  //     // }
  //     // if (state.checkedCompatBottomSPSet.count(std::make_pair(*spM,
  //     *bottomSP))){
  ...
  //       // state.checkedBadCompatBottomSPSet.insert(std::make_pair(*spM,
  //       *bottomSP)); continue;
  //     }
  ...
  //         can
  //         // be
  //         // described in terms of aCoef and bCoef, where v = aCoef * u +
  //         bCoef float uB = xVal / (xVal * xVal + yVal * yVal); float vB =
  //         yVal / (xVal * xVal + yVal * yVal);
  //         // in the rotated frame the interaction point is positioned at x =
  //         -rM
  //         // and y ~= impactParam
  //         float uIP = -1. / rM;
  ...
  //       }
  //     }
  //     // state.checkedCompatBottomSPSet.insert(std::make_pair(*spM,
  //     *bottomSP)); state.compatBottomSP.push_back(bottomSP);
  //   }
  //   // no bottom SP found -> try next spM
  //   if (state.compatBottomSP.empty()) {
  //     continue;
  ...
  //       // can do a loop to filter them even sooner
  //       // if (state.checkedCompatSeedSet.count(std::make_pair(*spM,
  //       std::make_pair(*state.compatBottomSP[b], *state.compatTopSP[t])))){
  //       //   state.nSeedsSkip += 1;
  //       //   continue;
  //       // }
  //       // if
  //       (state.checkedCompatSeedSet.count(std::make_pair((static_cast<const
  //       IndexSourceLink&>(*((spM.sourceLinks()[0])))).index(),
  //       std::make_pair(*state.compatBottomSP[b], *state.compatTopSP[t])))){
  //       // if
  //       (state.checkedCompatSeedSet.count(Seed<external_spacepoint_t>(state.compatBottomSP[b]->sp(),
  //       spM->sp(), state.compatTopSP[t]->sp(), 0))){
  //       //   state.nSeedsSkip += 1;
  //       //   continue;
  //       // }
  ...
  //         float dU = lt.U - Ub;
  //         if (dU == 0.) {
  //           // state.checkedCompatSeedSet.insert(std::make_pair(*spM,
  //           std::make_pair(*state.compatBottomSP[b],
  //           *state.compatTopSP[t])));
  //           //
  //           state.checkedCompatSeedSet.insert(std::make_pair((static_cast<const
  //           IndexSourceLink&>(*((spM.sourceLinks()[0])))).index(),
  //           std::make_pair(*state.compatBottomSP[b],
  //           *state.compatTopSP[t])));
  //           //
  //           state.checkedCompatSeedSet.insert(Seed<external_spacepoint_t>(state.compatBottomSP[b]->sp(),
  //           spM->sp(), state.compatTopSP[t]->sp(), 0)); continue;
  //         }
  //         // A and B are evaluated as a function of the circumference
  //         parameters
  //         // x_0 and y_0
  //         float A0 = (lt.V - Vb) / dU;
  ...
  //         float rotationTermsUVtoXY[2] = {spM->x() * sinTheta /
  //         spM->radius(),
  //                                         spM->y() * sinTheta /
  //                                         spM->radius()};
  ...
  //         rMxy =
  //             std::sqrt(rMTransf[0] * rMTransf[0] + rMTransf[1] *
  //             rMTransf[1]);
  ...
  //       // add errors of spB-spM and spM-spT pairs and add the correlation
  //       term
  //       // for errors on spM
  ...
  //       // (scatteringInRegion2). This assumes gaussian error propagation
  //       which
  //       // allows just adding the two errors if they are uncorrelated (which
  //       is
  //       // fair for scattering and measurement uncertainties)
  //       if (deltaCotTheta2 > (error2 + scatteringInRegion2)) {
  //         // skip top SPs based on cotTheta sorting when producing triplets
  //         if (m_config.skipPreviousTopSP) {
  //           // break if cotTheta from bottom SP < cotTheta from top SP
  //           because
  //           // the SP are sorted by cotTheta
  //           if (cotThetaB - cotThetaT < 0) {
  //             // for (size_t tNext = t; tNext < numTopSP; tNext++) {
  //             //   // state.checkedCompatSeedSet.insert(std::make_pair(*spM,
  //             std::make_pair(*state.compatBottomSP[b],
  //             *state.compatTopSP[tNext])));
  //             //
  //             state.checkedCompatSeedSet.insert(Seed<external_spacepoint_t>(state.compatBottomSP[b]->sp(),
  //             spM->sp(), state.compatTopSP[tNext]->sp(), 0));
  //             // }
  //             break;
  //           }
  //           t0 = t + 1;
  ...
  //       // refinement of the cut on the compatibility between the r-z slope
  //       of
  ...
  //       skip if (deltaCotTheta2 > (error2 + p2scatterSigma)) {
  //         if (m_config.skipPreviousTopSP) {
  //           if (cotThetaB - cotThetaT < 0) {
  //             // for (size_t tNext = t; tNext < numTopSP; tNext++) {
  //             //   // state.checkedCompatSeedSet.insert(std::make_pair(*spM,
  ...
  //         spM->sp(), state.compatTopSP[t]->sp(), 0)); continue;
  //       }
  //       // A and B allow calculation of impact params in U/V plane with
  //       linear
  //       // function
...
inline void
SeedFinderHashing<external_spacepoint_t, platform_t>::getCompatibleDoublets(
...
container_t<Seed<external_spacepoint_t>>
SeedFinderHashing<external_spacepoint_t, platform_t>::createSeedsForGroup(
    const Acts::SeedFinderOptions& options,
    const Acts::SpacePointGrid<external_spacepoint_t>& grid,
