...
logLevel = acts.examples.defaultLogging(s, None)()
...
if args.edm4hep:
    ...
    if args.output_csv:
        outputDirCsv = pathlib.Path(outputDir)
...
selector = acts.examples.ParticleSelector(
    **acts.examples.defaultKWArgs(
        **acts.examples.simulation._getParticleSelectionKWargs(
            ParticleSelectorConfig(
                # pt=(1.0 * u.GeV, None),
                # eta=(-3.0, 3.0),
                hits=(1, None),
    ...
    outputParticles="tmp_particles_hits",
)
...
if args.output_root:
    rootParticlesWriter = acts.examples.RootParticleWriter(
        level=logLevel,
        inputParticles="tmp_particles_hits",
        filePath=str(outputDir / "particles_hits.root"),
    )
...
if args.output_csv:
    ...
    s.addWriter(
        acts.examples.CsvParticleWriter(
            ...
            outputDir=str(outputDirCsv),
            outputStem="particles_digitized_selected",
        )
    )
...
if args.reco:
    ...
    if args.output_csv:
        csvSpacepointsWriter = acts.examples.CsvSpacePointWriter(
            level=acts.logging.INFO,
            inputSpacepoints="spacepoints",
            outputDir=str(outputDir),
        )
        s.addWriter(csvSpacepointsWriter)
