...
BOOST_AUTO_TEST_CASE(AnnoyTest) {
  ...
  for (const auto& arrayvec : test_vector) {
    /// Convert the vector to double*
    double* vec = (double*)malloc(f * sizeof(double));
    vec[0] = arrayvec[0];
    vec[1] = arrayvec[1];

    annoyModel.add_item(pointIndex, vec);
    pointIndex++;
  }
  ...
  bucketIds.clear();
  pointIndex = 0;
  annoyModel.get_nns_by_item(pointIndex, bucketSize, -1, &bucketIds, nullptr);
